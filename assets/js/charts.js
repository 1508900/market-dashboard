...
function renderCreditCharts() {
  ...
  function makeSpreadHistory(base, vol = 8) {
    return generateHistoricalSeries(base, 90, vol / base);
  }

  const igUS = makeSpreadHistory(usIg.spread, 6);
  const igEU = makeSpreadHistory(euIg.spread, 8);
  const hyUS = makeSpreadHistory(usHy.spread, 15);
  const hyEU = makeSpreadHistory(euHy.spread, 18);
